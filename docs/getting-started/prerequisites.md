...
!!! warning "Two version thresholds: v1.5.0 to start, v1.6.x to be supported"
    ...
    Being able to start is not the same as being supported: the controller is built and tested against the v1.6.2 standard bundle, and its GatewayClass `SupportedVersion` condition compares the installed bundle's `major.minor` against that version — any other minor, including v1.5.x, is reported as `SupportedVersion=False` with reason `UnsupportedVersion` while the controller keeps running.
