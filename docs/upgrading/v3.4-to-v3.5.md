...
v3.5 closes four ways a request, a tenant, or an unauthenticated caller could reach something that was not theirs, and stops one request's bug from taking the whole data plane down. There is no CR migration and no required values change, but the `GatewayClassConfig` CRD gains fields and must be re-applied by hand, and each change below can stop something that works today: a Gateway, a deployment's request routing, a hand-written proxy Deployment's startup, or a scraper that shares the controller's namespace. All are described below with what to do about them.
...
Arbitration reads `spec.tunnelID`, `spec.allowSharedTunnels` and `spec.maxDataPlanesPerNamespace` off the GatewayClassConfig, so a Gateway with its own data plane now needs that object to exist and be referenced by a usable `parametersRef`. The cap is unset by default and changes nothing on upgrade; see the [per-Gateway isolation guide](../guides/per-gateway-isolation.md) to turn it on. Before v3.5 the per-Gateway status path never touched the class chain, and a dedicated plane got status and a rendered Deployment regardless.
