...
v3.5 closes three ways a request, a tenant, or an unauthenticated caller could reach something that was not theirs, and stops one request's bug from taking the whole data plane down. There is no CRD migration and no required values change, but each of the three can stop something that works today: a Gateway, a deployment's request routing, or a hand-written proxy Deployment's startup. All are described below with what to do about them.
