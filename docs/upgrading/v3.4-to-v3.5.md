...
Affected: a values file that sets both `ipv4` and `ipv6` under `networkPolicy.cloudflareIpRanges` to empty lists. The chart ships both populated, so an install that never touched the field is unaffected, and one address family is enough — an IPv4-only or IPv6-only allowlist is a populated `to:` and still renders.

Two ways out. Either restore the ranges, keeping them current from [Cloudflare's published list](https://www.cloudflare.com/ips/), or drop the policy:
