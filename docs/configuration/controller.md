...
| `--proxy-image` | `CF_PROXY_IMAGE` | | Container image for per-Gateway rendered proxy Deployments ([per-Gateway isolation](../guides/per-gateway-isolation.md)). Empty disables rendering: Gateways referencing a `GatewayConfig` then surface an Event instead of a data plane. The Helm chart always sets it; manual installs MUST pass it to use per-Gateway data planes |
