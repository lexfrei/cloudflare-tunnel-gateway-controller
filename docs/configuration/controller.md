...
| `--controller-pod-selector` | `CF_CONTROLLER_POD_SELECTOR` | | Label selector (kubectl syntax) identifying this controller's own pods, AND'd with the controller namespace in the rendered per-Gateway NetworkPolicy; empty admits every pod in that namespace. The chart sets it from the controller's own labels |
| `--monitoring-namespace-selector` | `CF_MONITORING_NAMESPACE_SELECTOR` | | Label selector (kubectl syntax) for namespaces additionally allowed to reach a per-Gateway proxy's config-API/metrics port in the rendered NetworkPolicy; empty admits the controller pod only — see [Per-Gateway Isolation](../guides/per-gateway-isolation.md) |
