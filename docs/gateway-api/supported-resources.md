...
!!! note "Conformance runs through an edge-routing gRPC client"
    ...
    The upstream Gateway API gRPC conformance tests dial the Gateway address (`*.cfargotunnel.com`) directly, and that hostname resolves to Cloudflare's ULA range, which is not externally routable. The suite allows injecting a custom gRPC client, so the conformance run supplies one that dials the Cloudflare edge instead — the same approach the HTTP suite already uses via a custom `RoundTripper`. `GRPCExactMethodMatching`, `GRPCRouteHeaderMatching`, and `GRPCRouteNamedRule` run this way (the run pins the `http2` tunnel transport, since gRPC trailers do not survive QUIC). Two tests stay skipped: `GRPCRouteWeight`, whose distribution sampler news its own client and bypasses the injectable one ([kubernetes-sigs/gateway-api#4926](https://github.com/kubernetes-sigs/gateway-api/issues/4926)), and `GRPCRouteListenerHostnameMatching`, which needs the wildcard-hostname multi-label fix tracked in [#371](https://github.com/lexfrei/cloudflare-tunnel-gateway-controller/issues/371). gRPC routing is additionally validated end-to-end by the in-house e2e suite against a real tunnel.
