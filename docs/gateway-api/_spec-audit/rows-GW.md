...
| GW-82 | GatewayInfrastructure.Labels | CTRL | PARTIAL | render/render.go:129-145 resourceLabels propagates spec.infrastructure.labels onto rendered Deployment/Service/HPA + pod template; controller-owned keys applied last (cannot be spoofed) | Propagated for the opt-in per-Gateway data plane (#479). The shared chart-owned plane still has nothing per-Gateway to relabel, so the conformance SupportGatewayInfrastructurePropagation feature stays exempt (default plane, not opt-in). |
| GW-83 | GatewayInfrastructure.Labels | CTRL | PARTIAL | render/render.go:129-145; render_test.go TestProxyDeployment_InfrastructureLabelsPropagate | Per-Gateway Pods carry infra labels; changing them rolls the pods. Shared plane unchanged. |
| GW-84 | GatewayInfrastructure.Annotations | CTRL | PARTIAL | render/render.go:147-160 resourceAnnotations propagates spec.infrastructure.annotations onto rendered resources + pod template | Per-Gateway data plane only (opt-in); shared plane has no per-Gateway resources to annotate. |
