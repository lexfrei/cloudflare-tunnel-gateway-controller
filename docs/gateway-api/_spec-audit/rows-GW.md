...
| GW-82 | GatewayInfrastructure.Labels | CTRL | PARTIAL | render/render.go:159-173 resourceLabels propagates spec.infrastructure.labels onto rendered Deployment/Service/HPA + pod template; controller-owned keys applied last (cannot be spoofed) | Propagated for the opt-in per-Gateway data plane (#479). The shared chart-owned plane still has nothing per-Gateway to relabel, so the conformance SupportGatewayInfrastructurePropagation feature stays exempt (default plane, not opt-in). |
| GW-83 | GatewayInfrastructure.Labels | CTRL | PARTIAL | render/render.go:159-173; render_test.go TestProxyDeployment_InfrastructureLabelsPropagate | Per-Gateway Pods carry infra labels; changing them rolls the pods. Shared plane unchanged. |
| GW-84 | GatewayInfrastructure.Annotations | CTRL | PARTIAL | render/render.go:177-187 resourceAnnotations propagates spec.infrastructure.annotations onto rendered resources + pod template | Per-Gateway data plane only (opt-in); shared plane has no per-Gateway resources to annotate. |
| GW-85 | GatewayInfrastructure.ParametersRef | CTRL | MET | config/resolver_gateway.go ErrInvalidParameters (group/kind + NotFound/missing-key/garbled-token); gateway_controller.go resolveGatewayConfig + handleResolveError -> Accepted=False/InvalidParameters; render + GatewayInfraReconciler provision the dedicated data plane | A valid parametersRef now provisions a per-Gateway proxy + tunnel (#479); a missing/unsupported/malformed one surfaces Accepted=False/InvalidParameters. |
