...
| Gateway listeners description: traffic matching no listener hostname MUST be rejected — HTTP 404, gRPC Unimplemented | kubernetes-sigs/gateway-api#4408 | New controller-actionable clause, MET on both halves: HTTP no-match returns 404 (tested), and a gRPC client observes UNIMPLEMENTED — the proxy emits a bare HTTP 404 that the gRPC HTTP-to-status mapping converts client-side, verified with a real grpc-go client in TestGRPCClientObservesUnimplementedOnNoMatch (internal/proxy/grpc_nomatch_test.go). | Inventory GW-106; `rows-GW.md` GW-106. |
