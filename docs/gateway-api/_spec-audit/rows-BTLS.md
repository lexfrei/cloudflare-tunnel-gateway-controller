...
| POL-11 | MUST NOT | CTRL | MET | backendtlspolicy_controller.go statusGenerationStale on each own ancestor entry | An own ancestor entry whose stored conditions carry a newer observedGeneration is left as is instead of being overwritten by the older reconcile. |
