...
Audited against vendored `sigs.k8s.io/gateway-api v1.6.1` (originally at v1.5.1, refreshed per `00-compliance-matrix.md` "Baseline refreshes since v1.5.1"; no delta lands on a shared-type row here — the cross-type HTTPRoute/GRPCRoute hostname MUST→MAY relaxation, kubernetes-sigs/gateway-api#4598, is audited under GR-14/GR-15 in `rows-GR.md`, not SH). Class: CRD = enforced by kubebuilder/CEL markers; CTRL = controller logic; N/A = not applicable (reason in Notes). GEP-10..15 (route-attachment / per-parentRef status semantics) factored in for SH-08/09/14/15/44/45/54/56/58.
...
| SH-31 | should (lc) | CTRL | MET | route_status.go `buildAcceptedCondition` (only std RouteReason* used) | Accepted uses standard reasons (Accepted/Pending/NoMatchingParent/...) plus one implementation-specific rejection reason, `HostnameNotPermitted` (internal/hostnameownership/policy.go), permitted by the other-reasons clause. |
...
| SH-47 | MUST NOT | CTRL | MET | route_status.go foreign-entry preservation before the rebuild | Entries whose controllerName differs are carried over verbatim and their slots reserved before the own-entry truncation; pinned by the foreign-parent-entry survival test. |
| SH-49 | MUST | CTRL | MET | route_status.go (mergeOwnParentConditions, partitionPriorParents); listener_condition_transitions.go (preserveOwnedConditionTransitions) | Each own RouteParentStatus entry is merged over its prior entry (matched by full parentRef identity) via meta.SetStatusCondition, so an unchanged condition keeps its stored LastTransitionTime and the write is skipped entirely when nothing changed; dup-Type still cannot occur. |
...
| SH-51 | MUST NOT | CTRL | MET | route_status.go statusGenerationStale on each own RouteParentStatus entry | An own parent entry whose stored conditions carry a newer observedGeneration is left as is instead of being overwritten by the older reconcile. |
...
| SH-56 | MUST | CTRL | MET | route_status.go updateRouteParentStatuses | Own entries are rebuilt only for refs selecting managed Gateways; foreign entries are preserved verbatim alongside them. |
| SH-57 | MUST NOT | CTRL | MET | route_status.go foreign-entry preservation before the rebuild | A non-matching-controllerName entry survives the rebuild; same mechanism and test as SH-47. |
...
| SH-78 | should (lc) | CTRL | GAP | internal/controller/route_status.go `buildAcceptedCondition` (any per-parent sync error -> Status=False, Reason=Pending); internal/controller/route_syncer.go `errBrokenDataPlane` and `gatewaySyncError` (unresolved dedicated data plane; refused tunnel claim) | Documented deviation. The spec lists Pending among the reasons for Accepted=**Unknown** -- "used with the Accepted when a controller has not yet reconciled the route" (`shared_types.go:389-391`, reason table at `shared_types.go:357-363`) -- while this controller pairs Reason=Pending with Status=**False**. The pairing is deliberate only for the permanent causes: a refused tunnel claim (GW-107; `applyTunnelOwnership` clears the transient mark, so the refusal never retries) and a dedicated data plane whose resolve failed with a deterministic `config.ErrInvalidParameters` -- for those the route is served nowhere until the Gateway is fixed, and Status=False is the honest polarity since Unknown would read as "not looked at yet". The other two causes are retryable and are the GAP: a non-deterministic resolve failure is marked transient (`route_partition.go`) and requeued through `SyncResult.TransientBrokenKeys` (`route_syncer.go`), and a failed tunnel sync retries on the next reconcile -- for both, "the controller has not yet reconciled the route" under Status=Unknown is exactly what the spec describes. Predates v3.5 (`git show v3.4.2:internal/controller/route_status.go`, line 616 sets the same reason); v3.5 widened it to the refusal case. The reason NAME stays inside the standard enum (SH-31); recorded in limitations.md. |
