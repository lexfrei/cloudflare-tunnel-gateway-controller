...
WebSocket routes are naturally exempt: WS upgrades flow through the dedicated `proxyWebSocketUpgrade` path (because cloudflared's HTTP/2 response writer cannot be hijacked the way stdlib `httputil.ReverseProxy` expects), and that path bypasses the cached transport entirely. Once the upgrade completes, two `io.Copy` goroutines pipe bytes bidirectionally between the hijacked client conn and the backend conn. They run until either side closes its conn, or until the session carries no bytes in either direction for `proxy.websocket.idleTimeout` (1h by default) — a bound on abandonment, not on duration, since any traffic either way resets it. That bound covers the upgraded session only. When the backend refuses the upgrade, its non-101 response body is copied through with no deadline of any kind, so a backend that stalls part-way into that body still holds the goroutine and the tunnel stream open.
