...
- **`Accepted=False` / `Pending`** — the route bound to its parent, but the parent could not be programmed as written. The three causes do not clear the same way: a Gateway refused a tunnel it does not own stays this way until the claim is fixed, and says so on its own `Accepted` condition (`InvalidParameters`); a dedicated data plane that failed to resolve is equally stuck when the failure is a deterministic spec error, but a transient resolve failure is retried and recovers on a later sync without ever touching the Gateway's status; a failed Cloudflare tunnel sync also retries, and meanwhile the route keeps serving, because the in-cluster proxy is pushed its config regardless — the tunnel document only populates the Cloudflare dashboard's edge-routing view. Reporting `Pending` with `Accepted=False` is a documented deviation from the spec's reason table, which lists it under `Accepted=Unknown` for a route the controller has not reconciled yet — recorded as SH-78 in the [spec compliance matrix](_spec-audit/00-compliance-matrix.md).
- **`PartiallyInvalid=True`** (alongside `Accepted=True`) — some rules or backends are dropped while the route still serves the rest. The message starts with the spec-mandated `Dropped Rule` prefix and names the affected rule indices. This covers a single unsupported filter among several rules, an unsupported per-backend filter (only that backend's traffic fraction fails closed), an unparseable rule `timeouts` value (the rule serves without it), and a match pattern that does not compile (the rule is dropped, since a match that never compiles can serve nothing).
...
Separately, Gateway and ListenerSet listeners whose protocol this controller cannot serve (`TCP`, `TLS`, `UDP`, or any unrecognised protocol) are marked `Accepted=False, Reason=UnsupportedProtocol` on the listener status. The Gateway reflects that on its own `Accepted` condition per the Gateway API spec: a Gateway holding any such listener alongside at least one servable one stays `Accepted=True` but with `Reason=ListenersNotValid`, and a Gateway whose listeners are all unservable is `Accepted=False, Reason=ListenersNotValid`.
...
A `RequestMirror` filter is best-effort by specification, and the proxy keeps at most 64 mirror dispatches in flight per filter instance. Past that the copy is dropped rather than queued, because each in-flight dispatch pins its buffered request body — up to 1 MiB, above which the request is not mirrored at all — and a mirror backend that stops answering would otherwise let that memory grow with request rate. Budget around 64 MiB of proxy memory for each mirror filter a config carries; a burst of config pushes can exceed that briefly, since every push builds fresh filters while the previous generation's dispatches hold their bodies until they time out. The chart ships `proxy.resources.limits.memory: 512Mi`, so one mirror filter at its ceiling accounts for roughly an eighth of it, and a config carrying several of them wants that limit raised. Drops are logged at each power of ten per filter instance, so a mirror backend that is down leaves a usable signal rather than a line per dropped request, and the count restarts with each new generation of filters. The primary request is never affected.
...
## The ingress-rule budget is per tunnel, and it is ours

The controller refuses to write a tunnel document above 1000 ingress rules. **That figure is this controller's own cap, not one Cloudflare publishes**: Cloudflare's [account limits](https://developers.cloudflare.com/cloudflare-one/account-limits/) list 1,000 `cloudflared` tunnels per account and 1,000 routes per account (shared with Cloudflare Mesh), with no per-tunnel ingress-rule row, and their [configuration file reference](https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-tunnel/do-more-with-tunnels/local-management/configuration-file/), which documents the `ingress:` block, states no count cap on it. Read 1000 as a conservative line drawn here, not as a documented boundary. The configuration endpoint is a whole-document update, so that budget is shared by every namespace whose routes land on the tunnel. The document carries one rule per (route rule x hostname x path match), so the ceiling is reached in hostname and path combinations rather than in routes. **A single route can exceed it on its own, within the spec's own limits**: `spec.hostnames` allows 16 entries and one rule allows 64 matches, so 16 x 64 = 1024 rules from one route, before any other namespace has contributed anything. Across rules a route may reach 128 matches in total, so 16 x 128 = **2048 entries from one route** — more than twice the budget. That total-match rule is a plain CRD validation rather than an experimental one, so it holds in the Standard channel as well as Experimental.
...
A dedicated data plane removes the shared fate: it writes its own tunnel document, which the controller budgets separately, so one namespace's growth can no longer freeze another's. What it does not do is create headroom anyone has promised — the limits Cloudflare publishes for tunnels are account-scoped, so a fleet large enough to meet one of those is not relieved by adding tunnels. See [Per-Gateway Isolation](../guides/per-gateway-isolation.md).
...
### A dedicated Gateway keeps its `status.addresses` through a configuration error

A Gateway with its own data plane retains its `<tunnel-id>.cfargotunnel.com` address while reporting `Accepted=False`, where a Gateway on the shared plane clears it. The address is what records which tunnel that plane holds, and tunnel ownership is decided by it — clearing it during an ordinary token rotation would surrender the tunnel to the next claimant.

The visible consequence is for external-dns: it does not withdraw the DNS record when such a Gateway's configuration breaks, so the record outlives the failure and is removed only when the Gateway is deleted or opts out of its dedicated plane.

A Gateway refused by the tunnel-ownership rule reaches the same state, and there it is not a passing failure: the refusal stands until the contested tunnel is released, so the Gateway advertises — and external-dns publishes — a tunnel it will never serve. Deleting the Gateway or removing its `spec.infrastructure.parametersRef` withdraws the record.
...
The upstream conformance test `HTTPRouteBackendProtocolWebSocket` runs through the Cloudflare edge like the HTTP and gRPC suites: the conformance run supplies a tunnel-aware WebSocket dialer through the suite's injectable dialer option, rewriting the target to the edge hostname over `wss` and carrying the test's intended host in `X-Original-Host` (which the proxy honours only when the deployment sets `proxy.allowXOriginalHost`, as the conformance setup does) — the Gateway address itself (`<tunnel-id>.cfargotunnel.com`, AAAA records in Cloudflare's ULA `fd10::/8`) is unreachable from any test runner. `test/e2e/e2e_backend_protocol_websocket_test.go` additionally proves the WebSocket path end to end against the real tunnel hostname without the header pattern.
...
Startup-latency tradeoff of `auto`: because the `auto` choice must learn whether a GRPCRoute is present before dialing, an `auto`/unset proxy waits for the controller's first config push — bounded by a cap (~30s) — before establishing the tunnel. The controller pushes a config even on a route-less cluster (an empty one), so under normal operation the wait ends within seconds of the controller coming up; the full cap is only burned when no push arrives at all (for example, the controller itself is down or cannot reach the proxy). If you see an `auto` proxy slow to establish its tunnel, a missing first push is the cause; pin `proxy.tunnel.protocol: http2` or `quic` to dial immediately (an explicit transport skips the wait).
