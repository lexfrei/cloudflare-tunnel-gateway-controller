...
- **Check the quic-go fix on every rebase**: diff upstream cloudflared's own `replace github.com/quic-go/quic-go` line against ours (`chungthuang/quic-go v0.45.1-0.20250428085412-43229ad201fd`, `FIXME(#609)` above). Upstream has tried moving off this pin twice and reverted both attempts right before shipping — once before 2026.6.1, once before 2026.7.0 — so the fix does not land on its own; when their pin finally changes, move ours to match and close #609.
