...
- Backend WebSocket is supported via `Service.spec.ports[].appProtocol: kubernetes.io/ws` (and `kubernetes.io/wss` when paired with a `BackendTLSPolicy`). `httputil.ReverseProxy` natively hijacks the conn on the 101 upgrade response, so no transport switch is required. If `kubernetes.io/wss` is declared without a `BackendTLSPolicy` the proxy logs a WARN at conversion time and falls through to plaintext — the backend will then reject the upgrade. The proxy does not enforce the precondition; operators must attach the policy themselves.
- `spec.rules[].timeouts.request` and `timeouts.backend` are skipped only for routes whose backends were declared WebSocket-capable via `appProtocol: kubernetes.io/ws[s]` AND when the request itself carries `Connection: Upgrade` headers. Routes serving plain HTTP traffic always get their declared deadlines applied — a client that decorates a request with WebSocket headers cannot bypass timeouts on a route that has no WS-marked backend.
