...
func TestMerge_ListenerSetSummary_AllConflictedYieldsListenersNotValid(t *testing.T) {
	t.Parallel()
	...
	summary := res.ListenerSetSummary(ls)
	assert.False(t, summary.Accepted)
	assert.Equal(t, gatewayv1.ListenerSetReasonListenersNotValid, summary.Reason)
}

func TestMerge_ListenerSetSummary_AtLeastOneValidYieldsAccepted(t *testing.T) {
	t.Parallel()
	...
	ls := newListenerSet("ls1", "infra", "2026-01-01T00:00:00Z", []gatewayv1.ListenerEntry{
		{
			...
			Port:     80,
			Protocol: gatewayv1.HTTPProtocolType,
			Hostname: hostnamePtr("ok.example.com"),
		},
		{
			Name:     "ls-bad",
			Port:     80,
	...
	summary := res.ListenerSetSummary(ls)
	assert.True(t, summary.Accepted)
	assert.Equal(t, gatewayv1.ListenerSetReasonAccepted, summary.Reason)
}

func TestMerge_AttachedListenerSetsCount(t *testing.T) {
	t.Parallel()
	...
	good := newListenerSet("good", "infra", "2026-01-01T00:00:00Z", []gatewayv1.ListenerEntry{
		{
			Name:     "ok",
			Port:     80,
			Protocol: gatewayv1.HTTPProtocolType,
			Hostname: hostnamePtr("good.example.com"),
		},
	})
	bad := newListenerSet("bad", "infra", "2026-02-01T00:00:00Z", []gatewayv1.ListenerEntry{
		{
			Name:     "conflict",
			Port:     80,
			Protocol: gatewayv1.HTTPProtocolType,
	...
	res := listenermerge.Merge(gw, []*gatewayv1.ListenerSet{good, bad})

	assert.Equal(t, 1, res.AttachedListenerSets(), "only `good` has a non-conflicted listener")
}
