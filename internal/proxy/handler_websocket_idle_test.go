...
// often enough that a scheduler pause on a loaded runner cannot open a
// gap as wide as the bound under test.
//
...
func TestHandler_WebSocket_TunnelMode_BackendOnlyTrafficKeepsSessionOpen(t *testing.T) {
	...
	backend := newOneWayUpgradeBackend(t, 25*time.Millisecond)
	handler := newIdleWSHandler(t, backend.URL, 300*time.Millisecond)
