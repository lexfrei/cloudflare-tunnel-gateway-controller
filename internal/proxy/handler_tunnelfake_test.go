...
// /cloudflared/connection/connection.go) whose three deadline setters
// return nil without doing anything on either transport, because there
// is no socket on that side to act on -- the bytes ride an HTTP/2 or
// QUIC stream back to the edge. Reads and writes delegate.
//
...
// actuates on the client conn passes here while doing nothing over a
// tunnel.
