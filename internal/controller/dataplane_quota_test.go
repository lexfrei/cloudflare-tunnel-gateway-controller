...
func TestGatewayReconciler_DataPlaneQuota(t *testing.T) {
	...
	t.Run("a refused Gateway is not Programmed, and not for being invalid", func(t *testing.T) {
		...
		// The listener conditions are written by the same call, so a listener
		// saying Invalid while the Gateway says NoResources is one object
		// answering the same question two ways.
		require.NotEmpty(t, refused.Status.Listeners)
		listener := findCondition(refused.Status.Listeners[0].Conditions, string(gatewayv1.ListenerConditionProgrammed))
		require.NotNil(t, listener)
		assert.Equal(t, metav1.ConditionFalse, listener.Status)
		assert.Equal(t, string(gatewayv1.ListenerReasonPending), listener.Reason,
			"Pending is the spec's reason for a listener not yet online; Invalid claims a spec mistake")
	})
...
// TestGatewayReconciler_SiblingOptInEnqueuesTheDisplaced pins the trigger the
// cap needs and the other two rules do not. Ordering is oldest-first, so a
// Gateway that opts in with an OLDER creationTimestamp displaces the current
// newest holder — a verdict change on an object nobody wrote. Its own For()
// event never fires, and without this mapping the displaced Gateway keeps
// Accepted=True and a rendered plane until unrelated churn or the informer's
// resync, which is the divergence the shared claim set exists to prevent.
func TestGatewayReconciler_SiblingOptInEnqueuesTheDisplaced(t *testing.T) {
	t.Parallel()
	...
	assert.Contains(t, names, "tenant/gw-new",
		"the Gateway a sibling's opt-in displaces must be re-reconciled")
	assert.NotContains(t, names, "neighbour/gw-solo",
		"the cap is per namespace, so another namespace's Gateways are not woken")
}
