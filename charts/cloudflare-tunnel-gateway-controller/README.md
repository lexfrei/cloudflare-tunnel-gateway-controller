...
| networkPolicy.cloudflareIpRanges | object | `{"ipv4":["173.245.48.0/20","103.21.244.0/22","103.22.200.0/22","103.31.4.0/22","141.101.64.0/18","108.162.192.0/18","190.93.240.0/20","188.114.96.0/20","197.234.240.0/22","198.41.128.0/17","162.158.0.0/15","104.16.0.0/13","104.24.0.0/14","172.64.0.0/13","131.0.72.0/22"],"ipv6":["2606:4700::/32","2803:f800::/32","2405:b500::/32","2405:8100::/32","2a06:98c0::/29","2c0f:f248::/32"]}` | Cloudflare IP ranges for egress NetworkPolicy. Shared with the proxy policy's edge-egress rule. Source: https://www.cloudflare.com/ips/ Last updated: 2025-11-25 To update: curl https://www.cloudflare.com/ips-v4 && curl https://www.cloudflare.com/ips-v6  Emptying both lists is refused by whichever policy is switched on, because the rule built from them would render an empty `to:`, which NetworkPolicy reads as allow-all on the ports it carries. To drop the restriction, turn that policy off instead — networkPolicy.enabled for the controller policy, proxy.networkPolicy.egressRestricted for the proxy one. Each knob clears only its own policy's guard, so a deployment running both needs both. |
