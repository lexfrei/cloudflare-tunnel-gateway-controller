...
| gatewayClassConfig.allowSharedTunnels | bool | `false` | Permit a Gateway with a dedicated data plane to serve a Cloudflare Tunnel that another namespace's Gateway, or this GatewayClass itself, already serves. Off by default: a connector token proves nothing about the tunnel it names, so an unproven claim would otherwise collect a neighbour's routes. Such a Gateway is refused with Accepted=False/InvalidParameters instead. Turn this on only where every party on a shared tunnel is trusted to see the others' routes. |
...
| gatewayClassConfig.maxDataPlanesPerNamespace | int | unset (no cap) | Cap on how many Gateways in one namespace may each have a dedicated data plane. Every opted-in Gateway renders a proxy Deployment, a Service, a NetworkPolicy and an optional HPA into its namespace, and registers a connector on its tunnel. Past the cap the newest Gateways are refused with Accepted=False/DataPlaneQuotaExceeded and no plane is rendered for them; the oldest keep theirs. Lowering the cap below what a namespace already holds tears down the newest planes. Leave it unset for no cap; 0 is rejected, since it is what an operator writes for "no dedicated planes at all". |
...
| image | object | `{"digest":"","pullPolicy":"IfNotPresent","repository":"ghcr.io/lexfrei/cloudflare-tunnel-gateway-controller","tag":""}` | Container image configuration |
| image.digest | string | `""` | Image digest (`sha256:...`) to pin instead of a tag. Set it to deploy the exact artifact you verified with cosign, since a tag can be repointed afterwards. Takes precedence over `tag` when both are set. |
...
| networkPolicy | object | `{"cloudflareIpRanges":{"ipv4":["173.245.48.0/20","103.21.244.0/22","103.22.200.0/22","103.31.4.0/22","141.101.64.0/18","108.162.192.0/18","190.93.240.0/20","188.114.96.0/20","197.234.240.0/22","198.41.128.0/17","162.158.0.0/15","104.16.0.0/13","104.24.0.0/14","172.64.0.0/13","131.0.72.0/22"],"ipv6":["2606:4700::/32","2803:f800::/32","2405:b500::/32","2405:8100::/32","2a06:98c0::/29","2c0f:f248::/32"]},"enabled":false,"ingress":{"from":[]},"kubernetesApiIpBlocks":[]}` | NetworkPolicy configuration |
| networkPolicy.cloudflareIpRanges | object | `{"ipv4":["173.245.48.0/20","103.21.244.0/22","103.22.200.0/22","103.31.4.0/22","141.101.64.0/18","108.162.192.0/18","190.93.240.0/20","188.114.96.0/20","197.234.240.0/22","198.41.128.0/17","162.158.0.0/15","104.16.0.0/13","104.24.0.0/14","172.64.0.0/13","131.0.72.0/22"],"ipv6":["2606:4700::/32","2803:f800::/32","2405:b500::/32","2405:8100::/32","2a06:98c0::/29","2c0f:f248::/32"]}` | Cloudflare IP ranges for egress NetworkPolicy. Shared with the proxy policy's edge-egress rule. Source: https://www.cloudflare.com/ips/ Last updated: 2025-11-25 To update: curl https://www.cloudflare.com/ips-v4 && curl https://www.cloudflare.com/ips-v6  Emptying both lists is refused whenever networkPolicy.enabled or proxy.networkPolicy.egressRestricted is on — those two arm the guard, and neither is on by default. The rule built from these lists would otherwise render an empty `to:`, which NetworkPolicy reads as allow-all on the ports it carries. To drop the restriction, turn off whichever of those two is on. Each clears only its own policy's guard, so a deployment running both needs both. |
...
| networkPolicy.kubernetesApiIpBlocks | list | `[]` | Destinations the controller may reach on the Kubernetes API ports. EMPTY (the default) renders the rule with no `to`, which permits those ports to every destination. That is what this chart has always done, and it means THE CLOUDFLARE RANGES BELOW ARE INERT while it stays empty: egress rules are OR'd, so an unrestricted rule carrying TCP/443 permits 443 anywhere no matter what the Cloudflare rule says.  Narrowing it is the point of the field, but the chart cannot pick a default for you: an in-cluster ClusterIP, a node IP and a public managed endpoint are all normal places for an API server to live.    kubernetesApiIpBlocks:     - 10.0.0.0/8   # the node subnet serving the API endpoint  Use the address the traffic actually reaches. Most policy-enforcing CNIs evaluate egress after DNAT, so a Service ClusterIP never matches and the packet arrives at a node IP instead.  The default is empty rather than a catch-all CIDR because a catch-all is not equivalent to no restriction on every CNI: on some it denies the API server this controller has to reach, so it would break those clusters while pretending to restrict nothing. What a CIDR peer does and does not match is in the security reference under Egress Requirements. |
...
| proxy.allowXOriginalHost | bool | `false` | Trust the client-supplied `X-Original-Host` header as the routing key and the Host sent to the backend. TEST DEPLOYMENTS ONLY. The header exists so the Gateway API conformance suite can drive domains that are not registered on the Cloudflare account, which the edge would otherwise reject by Host. The edge forwards arbitrary `X-*` headers from any client, so enabling this in production lets a client that reaches one hostname be served by a different hostname's backend, with the intended hostname's edge policy never evaluated. |
...
| proxy.image | object | `{"digest":"","pullPolicy":"IfNotPresent","repository":"ghcr.io/lexfrei/cloudflare-tunnel-gateway-controller-proxy","tag":""}` | Proxy container image |
| proxy.image.digest | string | `""` | Image digest (`sha256:...`) to pin instead of a tag. Also carried into `--proxy-image`, so it becomes the default for the per-Gateway data planes the controller renders, not just the shared one — a Gateway whose `GatewayConfig` sets `spec.image` still overrides it. Takes precedence over `tag` when both are set. |
...
| proxy.networkPolicy.enabled | bool | `true` | Render the proxy NetworkPolicy (ingress-only; locks the config-API port to the controller pod by default) |
| proxy.networkPolicy.ingress | object | `{"from":[]}` | Ingress source configuration |
| proxy.networkPolicy.ingress.from | list | `[]` | EXTRA namespaces/pods allowed to reach the config-API port, ADDED alongside the controller pod — which is always admitted so a config push is never locked out. Its peer names the pod, not the whole release namespace, so a neighbour sharing that namespace needs listing here too. Add your monitoring namespace to let Prometheus scrape /metrics (locking the config-API port also restricts the /metrics it co-serves). |
| proxy.networkPolicy.monitoringNamespaceSelector | object | `{}` | Label selector for namespaces additionally allowed to reach the config-API/metrics port in the PER-GATEWAY (tenant) NetworkPolicies the controller renders (passed to --monitoring-namespace-selector). Empty = the controller pod only. For the SHARED proxy's policy, use `ingress.from` above instead. |
