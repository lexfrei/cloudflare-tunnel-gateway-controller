...
| image.digest | string | `""` | Image digest (`sha256:...`). When set it replaces `tag` in the rendered reference. See the Container Image Verification section of the security reference for when to set it. |
...
| proxy.image.digest | string | `""` | Image digest (`sha256:...`). When set it replaces `tag` in the rendered reference, and is passed to `--proxy-image` as the default image for per-Gateway data planes. See the Container Image Verification section of the security reference for what that default does and does not guarantee. |
